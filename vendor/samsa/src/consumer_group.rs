        ...
        async_stream::stream! {
            ...
            loop {
                ...
                */
                ...
                let fp = &self.fetch_params;
                let consumer = ConsumerBuilder::<T>::new(
                    self.connection_params.clone(),
                    assigned_topic_partitions,
                )
                .await?
                .max_wait_ms(fp.max_wait_ms)
                .min_bytes(fp.min_bytes)
                .max_bytes(fp.max_bytes)
                .max_partition_bytes(fp.max_partition_bytes)
                .isolation_level(fp.isolation_level)
                ...
                loop {
                    ...
                    tracing::debug!("Member {:?} | Heartbeat", self.member_id);
                    let hb = heartbeat(
                        coordinator_conn.clone(),
                        self.correlation_id,
                        &self.client_id,
